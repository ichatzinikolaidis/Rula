...
private:
	...
	void leftencoderCb(double enc);
	void rightencoderCb(double enc);
...
Odometry_calc::Odometry_calc() {
	...
	wheel_sub = n.subscribe("/drrobot_motor",10, &Odometry_calc::encodersCb, this);
...
void Odometry_calc::encodersCb(const jaguar4x4::MotorDataArray::ConstPtr& ticks) {
	...
	left_ticks = (deltaLF+deltaLR)/2;
	right_ticks = (deltaRF+deltaRR)/2;
	Odometry_calc::leftencoderCb(left_ticks);
	Odometry_calc::rightencoderCb(-right_ticks);
}

// Left encoder callback
void Odometry_calc::leftencoderCb(double enc) {
...
void Odometry_calc::rightencoderCb(double enc) {
	if((enc < encoder_low_wrap) && (prev_lencoder > encoder_high_wrap)) {
		rmult = rmult + 1;
	}
